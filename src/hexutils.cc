...
unsigned int IntelHexProgramFileType::getword(FILE *file)
{
...
void IntelHexProgramFileType::writeihexN(int bytes_per_word, Register **fr, int32_t size, FILE *file, int32_t out_base)
{
  ...
  if (fr == nullptr || file == nullptr || size <= 0
...
int IntelHexProgramFileType::readihex16(Processor *pProcessor, FILE * file)
{
  ...
  while (1) {
    ...
    checksum = 0;
    int bytesthisline = getbyte(file);
    int address = read_be_word(file) / 2;
     ...
     * 	RRR
     */
    int linetype = getbyte(file);	/* 0 for data, 1 for end  */
    ...
    int csby = getbyte(file);	/* get the checksum byte */
...
int IntelHexProgramFileType::readihexN(int bytes_per_word, Register **fr, int32_t size, FILE * file, int32_t offset)
{
  ...
  while (1) {
    ...
    checksum = 0;
    int bytesthisline = getbyte(file);
    int wordsthisline = bytesthisline  / bytes_per_word;
    int address = read_be_word(file);
    int linetype = getbyte(file);	/* 0 for data, 1 for end  */
