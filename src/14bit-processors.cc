...
//-------------------------------------------------------------------
//	Similar to pic_processor version except sets PCON flags
//
void _14bit_e_processor::reset(RESET_TYPE r)
{
   ...
   //     pcon.put(0x0d);
    ...
    case WDTWV_RESET:
        std::cout << "Reset due to WDTWV\n";
        pcon.put(pcon.get() & ~PCON::WDTWV);
        break;

    case WDT_RESET:
        std::cout << "Reset due to WDT\n";
        pcon.put(pcon.get() & ~PCON::RWDT);
        break;
...
bool _14bit_e_processor::set_config_word(unsigned int address, unsigned int cfg_word)
{
    ...
    if (address == 0x8007)   // Config Word 1
    {
        ...
        wdt->initialize((bool)(wdt_flag & 2));
