...
{
public:
    explicit Generic12bitConfigWord(P12bitBase *pCpu)
...
void P12bitBase::enter_sleep()
{
    ...
    std::cout << "enter sleep status=" << std::hex << status->get() << '\n';
}
...
void  P12bitBase::updateGP2Source()
{
    ...
    {
        std::cout << "TRIS now controlling gpio2\n";
...
void  P12bitBase::freqCalibration()
{
    ...
    if ((configWord & (FOSC0 | FOSC1)) == FOSC1)
    {
        ...
        freq *= 1.0 + 0.125 * osccal_val / 0x08;
        set_frequency(freq);
...
void  P12bitBase::option_new_bits_6_7(unsigned int bits)
{
    if (verbose)
    {
        std::cout << "P12bitBase::option_new_bits_6_7 bits=" << std::hex << bits << '\n';
    }
...
void P12bitBase::dump_registers()
{
    ...
    std::cout << "tris = 0x" << std::hex << m_tris->value.get() << '\n';
    std::cout << "osccal = 0x" << osccal.value.get()  << '\n';
}
...
void P12CE518::create()
{
    ...
    m_tris->put(0x3f);
    {
        ...
        io_scl = new IO_open_collector("gpio7");
    ...
    }
    {
        ...
        io_sda->setDrivingState(false);
        io_sda->setDriving(true);
        sda->attach_stimulus(m_gpio->addPin(io_sda, 6));
        sda->update();
    }
      ...
      io_sda(nullptr)
{
    if (verbose)
    {
        std::cout << "12CE518 constructor, type = " << isa() << '\n';
    }
...
void  P12CE518::freqCalibration()
{
    ...
    if ((configWord & (FOSC0 | FOSC1)) == FOSC1)
    {
        ...
        double freq = 4e6;
        freq *= 1.0 + 0.125 * osccal_val / 0x20;
        set_frequency(freq);
    ...
    : P12CE518(_name, desc)
{
    if (verbose)
    {
        std::cout << "12ce519 constructor, type = " << isa() << '\n';
    }
}
    ...
    : P12bitBase(_name, desc)
{
    if (verbose)
    {
        std::cout << "10f200 constructor, type = " << isa() << '\n';
    }
...
void  P10F200::freqCalibration()
{
    ...
    freq *= 1.0 + (0.125 * osccal_val) / 0x80;
    set_frequency(freq);
    ...
    : P10F200(_name, desc)
{
    if (verbose)
    {
        std::cout << "10f202 constructor, type = " << isa() << '\n';
    }
}
...
public:
    ...
    CMCON0(const CMCON0 &) = delete;
    CMCON0 & operator =(const CMCON0 &) = delete;
    ~CMCON0();

    virtual void put(unsigned int new_value) override;
    virtual void put_value(unsigned int new_value) override;
    bool isEnabled()
    {
...
{
public:
    explicit COUT_SignalSource(CMCON0 *pcmcon0)
        : m_cmcon0(pcmcon0)
    {}
    ~COUT_SignalSource() {}
...
public:
    ...
    virtual void setSinkState(char new3State) override
    {
        if (verbose)
            std::cout << "CIN_SignalSink::setSinkState  " << (m_binput ? "POS " : "NEG ")
                      << "set sink:" << new3State << '\n';
...
void CMCON0::setInputState(char /* newState */, bool bInput)
{
    ...
    if (verbose)
    {
        std::cout << "CMCON0::setInputState: pV=" << m_pV << ",nV=" << m_nV << '\n';
    }
...
P10F204::P10F204(const char *_name, const char *desc)
    : P10F200(_name, desc), m_cmcon0(nullptr)
{
}
...
void P10F204::updateGP2Source()
{
    ...
    else if (m_cmcon0->isEnabled())
    {
        ...
        std::cout << "comparator is controlling the output of GPIO2\n";
...
public:
    ...
    explicit P16F505ConfigWord(P12bitBase *pCpu)
        : ConfigWord("CONFIG", 0xfff, "Configuration Word", pCpu, 0xfff),
          m_pCpu(pCpu)
    {
        assert(pCpu);
...
void  P16F505::updateGP2Source()
{
    ...
    {
        std::cout << "TRIS now controlling PORTC5\n";
...
void  P16F505::option_new_bits_6_7(unsigned int bits)
{
    ...
    if (verbose)
    {
        std::cout << "P16F505::option_new_bits_6_7 bits=" << std::hex << bits << '\n';
    }
...
{
    _12bit_processor::dump_registers();
    std::cout << "trisb = 0x" << std::hex << m_trisb->value.get() << '\n';
    std::cout << "trisc = 0x" << std::hex << m_trisc->value.get() << '\n';
    std::cout << "osccal = 0x" << osccal.value.get()  << '\n';
}
