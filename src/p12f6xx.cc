...
public:
    ...
    virtual void set_tmr1if() override
    {
        put(get() | TMR1IF);
    }
    virtual void set_tmr2if() override
    {
        put(get() | TMR2IF);
    }
    ...
    virtual void set_nco1if() override
    {
    ...
    virtual void set_c1if() override
    {
        set_cmif();
    }
...

{
    ...
    tmr0.link_cpu(m_gpio, 4, option_reg);
    tmr0.start(0);
...
void P12F629::create_sfr_map()
{
    ...
    tmr1l.t1con = &t1con;
    /* 0.31.1gs */
    tmr1l.setInterruptSource(new InterruptSource(pir1, PIR1v12f::TMR1IF));
    tmr1h.tmrl  = &tmr1l;
    t1con.tmrl  = &tmr1l;
...

{
    ...
    /* 0.31.1gs */
    pir1->valid_bits |= PIR1v12f::CCP1IF;
    pir1->writable_bits |= PIR1v12f::CCP1IF;
}
...
void P12F683::create_sfr_map()
{
    ...
    pr2.tmr2    = &tmr2;
    /* 0.31.1gs */
    ccp1con.setCrosslinks(&ccpr1l, pir1, PIR1v12f::CCP1IF, &tmr2);
...
P10F32X::P10F32X(const char *_name, const char *desc)
    : _14bit_processor(_name, desc),
      ...
      fvrcon(this, "fvrcon", "Voltage reference control register", 0xf3),
...

{
    ...
    tmr0.link_cpu(m_porta, 3, option_reg);
    tmr0.start(0);
...
void P10F32X::create_sfr_map()
{
    ...
    add_sfr_register(&clc1.clcxsel0, 0x32, RegisterValue(0, 0), "clc1sel0");
    add_sfr_register(&clc1.clcxsel1, 0x33, RegisterValue(0, 0), "clc1sel1");
    add_sfr_register(&clc1.clcxpol, 0x34, RegisterValue(0, 0), "clc1pol");
    ...
    adcon0.setChannel_shift(2);
    adcon1.attach_ad_fvr(fvrcon.get_node_adcvref(), 7);
    adcon1.attach_Vt_fvr(fvrcon.get_node_Vtref(), 6);
    ...
    pwm1con.setIOpin(&(*m_porta)[0], CCPCON::CCP_PIN);
    pwm1con.set_tmr2(&tmr2);
    pwm1con.set_cwg(&cwg);
    pwm1con.set_clc(&clc1, 0);
    pwm2con.set_pwmdc(&pwm2dcl, &pwm2dch);
    pwm2con.setIOpin(&(*m_porta)[1], CCPCON::CCP_PIN);
