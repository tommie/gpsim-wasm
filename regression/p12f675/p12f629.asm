...
RESET_VECTOR  CODE    0x000              ; processor reset vector
  ...
  .sim "R2.ypos = 130"
   ...
   ; Use a pullup resistor to drive gpio2 for intpin test
  .sim "module load pullup R3"
  .sim "R3.resistance = 10000.0"
  .sim "R3.voltage = 5.0"
  .sim "R3.xpos = 252"
  .sim "R3.ypos = 180"

  .sim "node n1"
  .sim "attach n1 gpio2 gpio3 R3.pin"
  .sim "node n2"
...
test_intpin:
	...
	BANKSEL TRISIO
	movlw	0x04
	movwf	TRISIO  	; GPIO 2 output
	clrf	int_cnt

;	movlw	(1<<GIE) | (1<<INTE) | (1<<PEIE)
	movlw	(1<<INTE) | (1<<GIE)
	movwf	INTCON		; enable INT interrupt
   .command "R3.voltage=0.0"
	nop
	nop
   .command "R3.voltage=5.0"
	nop
	nop
   .assert "int_cnt == 1, '*** FAILED 12f629 no interrupt on +INT pin INTEDG=1'"
	nop
    	bcf OPTION_REG,INTEDG
   .command "R3.voltage=0.0"
	nop
   .assert "int_cnt == 2, '*** FAILED 12f629 no interrupt on -INT pin INTEDG=0'"
	nop
	return
...
;
	...
	movwf	INTCON		; enable T0 interrupt
