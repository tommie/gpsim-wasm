...
; <http://www.gnu.org/licenses/lgpl-2.1.html>.
        ...
        ;; Specifically, basic port operation, interrupts,
	;; a2d, dac, read config data, write to program memory


	list    p=16f1709                ; list directive to define processor
	include <p16f1709.inc>           ; processor specific variable definitions
...
MAIN    CODE
start
   .assert "p16f1709.frequency == 500000., 'FALIED 16F1709 POR clock 500kHz'"
	nop
...
;	bsf	ZCD1CON,ZCD1EN
	...
	goto	$-1
   .assert "(oscstat & 0x1f) == 0x19,  '*** FAILED 16f1709 HFIO bit error'"
	nop
   .assert "p16f1709.frequency == 16000000., 'FALIED 16F1709 RC clock 16MHz'"
	nop
	BANKSEL FVRCON
	...
	movlw	0xff
	movwf	PIR1
   .assert "pir1 == 0xcf,  '*** FAILED 16f1709  PIR1 writable bits'"
	nop
	clrf	PIR1
	movwf	PIR2
   .assert "pir2 == 0xef,  '*** FAILED 16f1709  PIR2 writable bits'"
	nop
	clrf	PIR2
	;
	...
	movlw	0xff
	movwf	PIE1
   .assert "pie1 == 0xff,  '*** FAILED 16f1709  PIE1 writable bits'"
	nop
	clrf	PIE1
	movwf	PIE2
   .assert "pie2 == 0xef,  '*** FAILED 16f1709  PIE2 writable bits'"
	nop
	clrf	PIE2
;	;
	...
	clrf	STATUS
	clrf	TRISA
   .assert "trisa == 0x08, '*** FAILED 16f1709  TRISA not clear except bit 3'"
	nop
	BANKSEL PORTA
	movlw	0xff
	movwf	PORTA
   .assert "porta == 0x28, '*** FAILED 16f1709  analog bits read 0'"
	nop
	movf	PORTA,W
...
;
  ...
  .assert  "'*** PASSED 16f1709 Functionality'"
	nop
	reset
	goto	$
...
test_a2d
	...
	movwf   ADCON0      	
	call	a2dConvert
   .assert "adresh == 0x33, '*** FAILED 16f1709 AN4=1V'"
	nop
  ; measure Core Temperature
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x02) && (adresl==0x2f), '*** FAILED 16f1709 ADC Core Temp'"
	nop
  ...
  ; measure FVR 
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x01) && (adresl==0xa3), '*** FAILED 16f1709 ADC FVR'"
	nop
  ...
  ; measure FVR using FVR Reference
	...
	movwf	ADCON0
	call	a2dConvert
   .assert "(adresh==0x03) && (adresl==0xff), '*** FAILED 16f1709 ADC FVR with FVR Ref'"
	nop
...
eefail:
  .assert "'***FAILED 16f1709 eerom write/read error'"
	nop
...
test_int:
	...
	movlw	0xff
	movwf	IOCAF
   .assert "iocaf == 0x3f, '*** FAILED 16f1709 INT test - IOCAF writable bits'"
	nop
	clrf	IOCAF		; Clear intcon IOCIF flag 
	movwf	INTCON
   .assert "intcon == 0xf8, '*** FAILED 16f1709 INT test - INTCON:IOCIF read only'"
	nop
	clrf	INTCON
	...
	BANKSEL PORTA
        ...
        nop
   .assert "iocaf_val == 0x04, '*** FAILED 16f1709 IOCAF bit 2 not set - No int on rising edge'"
	nop
   .assert "(intcon & 1) == 0, '*** FAILED 16f1709 INTCON,IOCIF not cleared in int'"
	nop 
	BANKSEL	IOCAF
	bcf	IOCAF,2
   .assert "(intcon & 1) == 0, '*** FAILED 16f1709 INTCON,IOCIF not cleared from IOCAF'"
	nop 
	clrf	iocaf_val
	...
	BANKSEL	PORTA
        ...
        nop
        movf    iocaf_val,w
   .assert "iocaf_val == 0x00, '*** FAILED 16f1709 INT test - Unexpected int on falling edge'"
        nop
...
;	Setup - edge interrupt
	...
	clrf	iocaf_val
        bsf     PORTA,5          ; make a rising edge
        movf    iocaf_val,w
   .assert "W == 0x00, '*** FAILED 16f1709 INT test - Unexpected int on rising edge'"
        nop
        clrf    iocaf_val
        bcf     PORTA,5          ; make a falling edge
        nop
   .assert "iocaf_val == 0x04, '*** FAILED 16f1709 IOCAF bit 2 not set'"
	nop
	BANKSEL	IOCAF
	CLRF	IOCAF
	bsf	INTCON,GIE
...
read_config_data:
   ...
   .assert "pmdath == 0x30 && pmdatl == 0x54, '*** FAILED 16f1709 Device ID'"
	nop
  ...
  .assert "UserID2 == 0x3054, '*** FAILED 16f1709 write to UserID2'"
	nop
	return
