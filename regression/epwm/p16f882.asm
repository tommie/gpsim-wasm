...
full_forward
    ...
    movwf  CCP1CON       ;
    banksel PORTC
    btfss   PIR1,TMR2IF  ; wait for new cycle
    goto    $-1
    clrf   TMR0		 ; start T0 time
    btfsc  PORTB,4	 ; wait for B4 low
    goto   $-1
  .assert "tmr0 == 0x1f, \"*** FAILED 16f882 PWM full forward output high time\"" 
    nop
    btfss  PORTB,4	; wait for next B4 high
    goto   $-1
  .assert "tmr0 == 0x2f, \"*** FAILED 16f882 PWM full forward period\"" 
    nop
    return
...
setup: 
    ...
    movlw  0xc0
    movwf  INTCON        ;  Disable interrupts and clear T0IF
    banksel TRISC
    ; Make output pins
    ...
    bsf	   PIE1,CCP1IE
    movlw  0x01		 ; Port B pull-up  Tmr0 internal clock prescaler 4
    movwf  OPTION_REG
    banksel PIR1         ;  Bank0
