...
;   .sim "V1.voltage = 5."
   ...
   .sim "node na0"
   .sim "attach na0 V3.pin porta0"	; C12IN0-
   .sim "node na1"
   .sim "attach na1 V1.pin porta1"	; C12IN1-

   .sim "node na3"
   .sim "attach na3 V2.pin porta3"	; C1IN+
...
test_tot1:
	...
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, '***FAILED p16f882 T1 running gate off from SYNCC2out'"
	nop
	...
	nop
	SUBWF	TMR1L,W
  .assert "W != 0, '***FAILED p16f882 T1 gate invert SYNCC2out'"
	nop
	BANKSEL CM2CON0
	...
	nop
	SUBWF	TMR1L,W
  .assert "W == 0, '***FAILED p16f882 T1 running gate on c2POL=0'"
	nop
	return
...
test_compar:
  ...
  .assert "cm2con1 == 0x33, '***FAILED 16f882 cm2con1 writable bits'"
	nop
        clrf	CM2CON1
   ;; with ON=0 POL should toggle output
  .assert "cm1con0 == 0x00, '***FAILED 16f882 cm1con0 off POL=0'"
	nop
 
	bsf	CM1CON0,C1POL	; toggle ouput polarity, not ON
  .assert "cm1con0 == 0x50, '***FAILED 16f882 cm1con0 off toggle'"
	nop
  .assert "cm2con1 == 0x80, '***FAILED 16f882 cm2con1 mirror C1 C2 off'"
	nop
  ...
  .assert "cm2con1 == 0xc0, '***FAILED 16f882 cm2con1 mirror C2OUT'"
	nop
	bsf	CM1CON0,C1OE	; C1OUT to RA4 on
   .assert "(porta & 0x30) == 0x30, '***FAILED 16f882 compare RA3(+)<RA0(-1) POL=1 RA4(C1OUT)=1'"
	nop 
	bcf	CM1CON0,C1POL	; toggle ouput polarity
   .assert "(porta & 0x30) == 0x20, '***FAILED 16f882 compare C1OUT(RA4) low'"
	nop
	; Test change in voltage detected
   .command "V3.voltage = 2.0"
	nop
   .assert "(porta & 0x10) == 0x10, '***FAILED 16f882 compare C1OUT(RA4) not high'"
	nop
